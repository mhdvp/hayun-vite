...
import units from './units_Box_New.js';

export default class Audiogram {
	constructor({ box, side, events = true }) {

		const { container, width, height } = box
	...
	drawSymbolsChart() {
		const { width, height } = units.symbolsChart
		let style;
		let x = 0
		let y = 0
		...
		style = 'fill: none;'
		putRect({ container: svg, x, y, width, height, style });
		...
		(this.side === 'R' ? ['R_AC', 'R_BC', 'R_AC_M', 'R_BC_M'] : ['L_AC', 'L_BC', 'L_AC_M', 'L_BC_M'])
			.forEach(symbolName => {
				...
				rect[symbolName] = putRect({ container: svg, x, y, width, height, name: symbolName, style });
				this.insertSymbol({ container: svg, symbolName, x: x + dw / 2, y: dh / 2 })
				x += dw;
			});
